...
export default class extends Controller {
     ...
     * offers.
     *
     * The topbar holds every form, the unused ones hidden: the pl mark inline,
     * repainted stroke by stroke from the colourway tile's JSON (light or dark
     * chrome, whichever is on — geometry-blind, indexed the way
     * _logo_mark.html.twig draws), and every other icon as two images from the
     * logo route, one per chrome, of which the stylesheet shows one.
     *
     * The tab icon's URL IS the choice (see _favicon.html.twig): browsers keep
     * a favicon cache that ignores ordinary revalidation, so a new choice
     * needs a new href, which is also what makes the tab repaint right now
     * instead of after a hard reload. That href is set here, before save()
     * has sent anything, which is safe only because the route draws what the
     * URL names and never what the session holds.
     */
    repaintLogo() {
        ...
        document.querySelectorAll('[data-logo-live-icon]').forEach((image) => {
            ...
            if (!classic) {
                const form = image.dataset.logoLiveIcon === 'dark' ? 'logoBarDark' : 'logoBar';
                this.setIfChanged(image, 'src', this.iconUrl(motif, paint, form));
            }
        });
        ...
        document.querySelectorAll('link[rel="icon"][type="image/svg+xml"]').forEach((link) => {
            this.setIfChanged(link, 'href', this.iconUrl(motif, paint, 'logoFavicon'));
        });
    }

    /**
     * One motif in one paint, as a tile (the default) or in another of the
     * forms its button carries a URL for: `logoFavicon`, `logoBar`,
     * `logoBarDark`.
     *
     * Built from the motif button's own URL rather than from a pattern: that
     * URL IS the route for the motif in its original design, generated by the
