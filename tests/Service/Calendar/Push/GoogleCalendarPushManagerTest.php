...
final class GoogleCalendarPushManagerTest extends KernelTestCase
{
    ...
    /**
     * A holiday or birthday calendar is generated by Google rather than stored,
     * and events.watch refuses it with pushNotSupportedForRequestedResource. It
     * is a fact about the calendar, not a transient failure, so asking again
     * every hour writes an unactionable warning an hour for ever — which is how
     * the log lines that DO matter stop being read. Seen in production the day
     * push shipped.
     */
    public function testACalendarGoogleWillNeverWatchIsNotAskedAgain(): void
    {
        $manager = $this->manager([new MockResponse(
            json_encode([
                'error' => ['code' => 400, 'errors' => [[
        ...
        self::assertFalse(
            $manager->supports($this->calendar),
            'the sweep must stop offering a calendar Google has already refused',
        );
        self::assertSame([], $this->logger->matching('warning', 'staying on polling'), 'and stop warning about it');
    }
