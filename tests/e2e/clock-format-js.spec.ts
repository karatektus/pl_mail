...
test.describe("the clock setting reaches the browser", () => {
    ...
    /**
     * The event editor's times. They were the browser's own `datetime-local`,
     * which writes the time in the BROWSER's locale whatever the page says —
     * "03:00 PM" beside a setting that asked for 15:00. See calendar--when,
     * whose cards and hour grid are drawn on the chosen clock.
     */
    test("the event editor writes its times on the chosen clock", async ({ page }) => {
        const startTime = () =>
            page.locator('[data-calendar--when-target="startCard"] [data-when-part="time"]');

        await chooseClock(page, "24");
        await page.goto("/calendar/day");
        await page.getByRole("button", { name: /New event on/ }).first().click();
        await expect(startTime()).toHaveText(/^\d{2}:00$/);

        await chooseClock(page, "12");
        await page.goto("/calendar/day");
        await page.getByRole("button", { name: /New event on/ }).first().click();
        await expect(startTime()).toHaveText(/^\d{1,2}:00 (am|pm)$/);
    });
});
