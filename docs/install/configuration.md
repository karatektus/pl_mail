...
| `MERCURE_COOKIE_NAME` | The name of the cookie that lets the browser subscribe. The app sets it and the hub reads it, both from this one variable. | blank — `__Secure-mercure_access_token` | Only without TLS | An install serving plain HTTP, with no TLS anywhere in front, needs a name without the `__Secure-` prefix, such as `mercure_access_token`: a browser drops a `__Secure-` cookie that did not arrive over HTTPS, and live updates never start. |
...
| `APP_DB_LOG_LEVEL` | Minimum Monolog level kept in the database for the admin log browser. | `warning` | No | The **default** — Admin → Logs can override it without a restart, and does so until it is set back to *from the environment*. Lowering it to `info` or `debug` fills the `log_entry` table quickly; `app:monitoring:prune` keeps 14 days by default, which is a lot of rows at debug. |
...
| `PUSH_ALLOWED_HOSTS` | Comma-separated hosts a push endpoint may point at although they resolve to a private address. | blank | Only for a LAN ntfy | Every other private, loopback or link-local push endpoint is refused at registration and at send. Running the bundled ntfy on a LAN or Tailscale address, set it to your `SERVER_NAME` or phones stop receiving pushes. See [the SSRF guard](#the-ssrf-guard) below. |
...
Three variables relax one check, and it is worth being exact about what the check is for.
...
resolves the host and refuses any address in a private, loopback, link-local or reserved range —
Symfony's `IpUtils::PRIVATE_SUBNETS`, which includes IPv4-mapped IPv6 — and refuses `http://`
outright. Resolving is what catches container names such as `database`, wildcard DNS such as
`127.0.0.1.nip.io`, and spellings such as `127.1` or `2130706433`.

That check gives the user a readable error when they save. The requests themselves go through
`App\Infrastructure\Http\UserUrlHttpClient`, which wraps Symfony's `NoPrivateNetworkHttpClient`:
it resolves the host again, pins the address it checked, and re-checks every redirect. The same
client carries CalDAV, ICS feeds and push deliveries.
...
- **`PUSH_ALLOWED_HOSTS=192.168.1.10`** does the same for push endpoints, which are otherwise
  refused on a private address unconditionally. It exists for the bundled ntfy, whose endpoints are
  `http://$SERVER_NAME:8090`. Push deliveries never follow a redirect.
...
ignored, including a stale one saved before the pin. A pinned host is exempt from the private-range
block without being listed, so pinning `http://nextcloud` for the container next door works.

**The failure mode is an exemption that follows the first host only.** A listed or pinned host may
redirect within its own addresses; a public server redirecting *to* that host's address is still
refused. A LAN service that redirects to a second internal name needs that name listed too.
